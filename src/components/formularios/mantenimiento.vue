<template>
  <div class="app">
    ...
    <div class="login-box">
      <form>
        <div class="user-box">
          <select required v-model="selectedOptionM">
        ...
        <div class="user-box">
          <input type="text" name="" required="" v-model="Responsable" />
        ...
        <center>
          <button @click.prevent="Mantenimiento()">Registrar</button>
        </center>
      </form>
    </div>
    <div class="cont_btn">
...
let selectedOptionM = ref("");
let Fecha_Mantenimiento = ref("");
let Descripcion = ref("");
let Responsable = ref("");
let Precio_Mantenimiento = ref("");
...
async function Mantenimiento() {
  try {
    let maquina = () => {
      let selectedMaquina = maquinas.value[selectedOptionM.value - 1];
      return selectedMaquina._id;
    }

    let maquina_id = maquina();
    ...
    if (mantenimiento.maquina_id === ""){
      text.value = "Seleccione una maquina";
    ...
    if (mantenimiento.fecha === ""){
      text.value = "Ingrese la fecha de mantenimiento";
      registroFallido.value = true;
      ocultar();
      return;
    }
    ...
    if (mantenimiento.descripcion === ""){
      text.value = "Ingrese la descripcion";
      registroFallido.value = true;
      ocultar();
      return;
    }

    r = await useMantenimientos.postMantenimiento(mantenimiento);
    registroExitoso.value = true;
    ocultar();
  }
  catch (error) {
    // text.value = "Error al registrar el mantenimiento";
    // registroFallido.value = true;
    // ocultar();
    console.log(error);
    return error;
