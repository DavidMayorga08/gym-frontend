...
let columns = ref([
  { name: "nombre", label: "Nombre Cliente", align: "center", field: "nombre" },
  {
    ...
    align: "center",
    field: (row) => row.fechaNacimiento.split("T")[0],
  },
  ...
  { name: "estado", label: "Estado", align: "center", field: "estado" },
  {
    ...
    field: (row) => {
      ...
      return plan ? plan.descripcion : "Sin plan";
    },
  },
  {
    name: "seguimiento",
...
let ver = async (row) => {
  ...
  fechaNacimiento.value = row.fechaNacimiento.split("T")[0];
  edad.value = row.edad;
  fechaIngreso.value = row.fechaIngreso.split("T")[0];
  ...
  imc.value = row.seguimiento[0].imc;
  medidaBrazo.value = row.seguimiento[0].medidaBrazo;
  medidaPierna.value = row.seguimiento[0].medidaPierna;
  medidaCintura.value = row.seguimiento[0].medidaCintura;
  Id.value = row._id;
};
