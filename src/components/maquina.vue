...
let ver = async (row) => {
  ...
  Fecha_Ingreso.value = row.fechaIngreso.split("T")[0];
  Fecha_Ultimo_Mantenimiento.value = row.fechaUltimoMantenimiento.split("T")[0];
...
let modificarMaquina = async () => {
  ...
  try {
    ...
    let maquina = {
      ...
      fechaUltimoMantenimiento: Fecha_Ultimo_Mantenimiento.value,
      estado: Estado.value,
    };
    ...
    await useMaquinas.putMaquina(Id.value, maquina);
